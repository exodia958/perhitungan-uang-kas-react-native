    ...
    <div class="row justify-center">
      <q-card class="q-mb-md bg-light-blue-4" style="width: 80%;">
        <q-card-section style="text-align: center;">
          ...
          <p class="text-weight-bold">jumlah debit : S/.{{ totalExpense.toFixed(2) }}</p>
          <p class="text-weight-bold">TOTAL : S/.{{ totalIncomes.toFixed(2) }}</p>
...
<script setup lang="ts">
  ...
  const columns = [
  { name: 'amount', required: true, label: 'jumlah', align: 'left', field: (row: { amount: number; }) => 'S/. ' + row.amount, format: (val: any) => `${val}`, sortable: true,
  ...
  { name: 'type', align: 'center', label: 'TIPE', field: (row: { type: string; }) => row.type == 'expense' ? 'egreso' : 'pemasukan' , sortable: true},
  { name: 'date', align: 'center', label: 'Tanggal', field: 'date'},
]
