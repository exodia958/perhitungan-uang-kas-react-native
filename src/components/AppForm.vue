<template>
  <q-form @submit="saveForm">
    <div class="text-h6" style="text-align:center;">tambah {{ props.name }}</div>
    <div class="q-mb-sm">
      <q-input outlined type="number" step=".01" v-model.number="amount" label="jumlah"
        :rules="[val => val && val !=0 || 'masukkan jumlah']" ref="amountref" @click="selectinput" lazy-rules/>
    </div>
    <div v-if="props.category" class="q-mb-sm">
      <q-select ref="categoryref" outlined v-model="categoryselected" :options="categoriesnew" label="kategori"
        :rules="[val => val || 'pilihkategori']" lazy-rules/>
    </div>
    <div class="q-mb-sm">
      <q-input ref="descriptionref" outlined v-model="description" label="deskripsi"/>
    </div>
    <div v-if="name == 'pemasukan'">
    ...
    <div class=" row justify-center">
      <q-btn color="primary" :label="`Tambah ${ props.name }`" type="submit"/>
    </div>
  </q-form>
</template>
  ...
  const saveForm = () => {
    try {
      ...
      const newOnetransaction: newtransaction = {
        ...
        type: props.name == 'pengeluaran' ? 'expense' : 'income'
      }
